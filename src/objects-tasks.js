...
function shallowCopy(obj) {
  const o = {};
  const objAnswer = Object.assign(o, obj);
  return objAnswer;
}
...
function mergeObjects(objects) {
  ...
  newArray.forEach((item) => {
    if (myMap.has(item[0])) {
      myMap.set(item[0], myMap.get(item[0]) + item[1]);
    } else myMap.set(item[0], item[1]);
  });
...
function compareObjects(obj1, obj2) {
  ...
  for (let i = 0; i < keysObj1.length; i += 1) {
    if (!obj2[keysObj1[i]] || obj1[keysObj1[i]] !== obj2[keysObj1[i]]) {
      return false;
    }
  }
  return true;
